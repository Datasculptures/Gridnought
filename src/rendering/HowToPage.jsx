...
export default function HowToPage({ visible, onBack }) {
  ...
  useEffect(() => {
    ...
    const gridSize = 230;
    const grid = new THREE.GridHelper(gridSize, 92, 0x00aa00, 0x004400);
    scene.add(grid);
    ...
    const streetX = -64;
    const rowZ    = [-88, -72, -56, -40, -24, -8, 8, 24, 40, 56, 72, 88];
    const byLetter = Object.fromEntries(BUILDING_TEMPLATES.map(t => [t.letter, t]));
    const districtRows = [
      { x: streetX - 14, ry:  Math.PI / 2, letters: ['A', 'C', 'D', 'K', 'F', 'J', 'M', 'O', 'Q', 'S', 'U', 'W'] }, // west curb, faces east
      { x: streetX + 14, ry: -Math.PI / 2, letters: ['B', 'G', 'I', 'E', 'L', 'H', 'N', 'P', 'R', 'T', 'V', 'X'] }, // east curb, faces west
    ];
